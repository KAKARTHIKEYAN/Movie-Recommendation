$(function() {
  ...
  $('.movie-button').on('click',function(){
    console.log("Button axtion is clicked ");
    var my_api_key = '8ae25301f556f345ee018fa6654ee203';
    var title = $('.movie').val();
    console.log("&&&&&&&++++++++++++++++++++++",title);
    if (title=="") {
      console.log("Button axtion is clicked your title is empty ");
      $('.results').css('display','none');
      $('.fail').css('display','block');
    }
    else{
      console.log("load_details is going to load  ");
...
function recommendcard(e){
  console.log("recommendcard is fired  ");
  var my_api_key = '8ae25301f556f345ee018fa6654ee203';
...
function load_details(my_api_key,title){
  console.log("load_details is called first line  ");
  $.ajax({
    type: 'GET',
    ...
    success: function(movie){
      ...
      if(movie.results.length<1){
        console.log("load_details =========== movie result is less than 1");
        $('.fail').css('display','block');
        $('.results').css('display','none');
        $("#loader").delay(500).fadeOut();
      }
      else{
        console.log("load_details function else block ");
        $("#loader").fadeIn();
        $('.fail').css('display','none');
        $('.results').delay(1000).css('display','block');
        var movie_id = movie.results[0].id;
        var movie_title = movie.results[0].original_title;
        console.log("load_details++++++++++++++",movie_title,movie_id);
        console.log("movie_recs is going to be called ");
        movie_recs(movie_title,movie_id,my_api_key);
      }
    },
    error: function(){
      console.log("load_details error function ");
...
function show_details(movie_details,arr,movie_title,my_api_key,movie_id){
  console.log("show_details  first is called ");
  ...
  $.ajax({
    ...
    success: function(response) {
      console.log("show movie details is called response is reciebed")
